...
@Suppress("unused")
/**
 * Resolver for [CustomerEntity]s. Provides access to properties which the GraphQL schema exposes of these objects, but which
 * don't exist directly on the domain model object (CustomerEntity in this case), and need to be queried for separately. This
 ...
 */
class CustomerResolver : DataLoadingResolver(), GraphQLResolver<CustomerEntity> {

    /**
     * Gets a [CompletableFuture] which, when completed, will return the company for the passed in customer.
     */
    fun company(customer: CustomerEntity, env: DataFetchingEnvironment): CompletableFuture<Company> =
     ...
     */
    fun outOfOfficeDelegate(customer: CustomerEntity, env: DataFetchingEnvironment): CompletableFuture<CustomerEntity?> =
     ...
     */
    fun pricingDetails(customer: CustomerEntity, env: DataFetchingEnvironment): CompletableFuture<PricingDetails> =
     ...
     */
    fun orders(customer: CustomerEntity, env: DataFetchingEnvironment): CompletableFuture<List<Order>> =
